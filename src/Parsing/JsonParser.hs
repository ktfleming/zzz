...
newtype JsonKey = JsonKey T.Text deriving (Show)

-- Values that can show up on the right side of a ':'
data JsonValue =
    JsonStringValue T.Text
  | JsonNumber T.Text
  | JsonBool Bool
  | JsonNull deriving (Show)

-- We need to preserve the whitespace generated by aeson-pretty, so it gets stored in this newtype.
newtype Indentation = Indentation T.Text deriving (Show)
...
type JsonParser = Parsec Void T.Text
